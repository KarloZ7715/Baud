...
fn test_selected_text_hello_world() {
    ...
    assert_eq!(text, "hello world", 
        "selected_text() debe devolver 'hello world', obtuvo: {:?}", text);
}
...
fn test_selected_text_multiline() {
    ...
    let text = term.selected_text();
    assert!(text.contains("line1"), "debe contener line1, obtuvo: {:?}", text);
    assert!(text.contains("line2"), "debe contener line2, obtuvo: {:?}", text);
    assert!(text.contains("line3"), "debe contener line3, obtuvo: {:?}", text);
}
