...
pub struct Parser<const OSC_RAW_BUF_SIZE: usize = MAX_OSC_RAW> {
    ...
    /// True while inside an APC string (`ESC _` … ST), so bytes are delivered
    /// via [`Perform::apc_put`]. SOS (`ESC X`) and PM (`ESC ^`) share the same
    /// parser state but do not set this flag.
    in_apc: bool,
}
...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    fn change_state<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        match self.state {
            ...
            State::SosPmApcString => self.advance_sos_pm_apc(performer, byte),
    ...
    #[inline(always)]
    fn advance_sos_pm_apc<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        match byte {
            0x18 | 0x1A => {
                if self.in_apc {
                    performer.apc_end();
...
pub trait Perform {
    ...
    /// Called at the start of an APC sequence (`ESC _`).
    fn apc_start(&mut self) {}

    /// Pass a byte belonging to the current APC string.
    fn apc_put(&mut self, _byte: u8) {}

    /// Called when an APC string is terminated (ST, CAN or SUB).
    fn apc_end(&mut self) {}
...
mod tests {
    ...
    #[test]
    fn parse_apc_delivers_bytes() {
        let mut dispatcher = Dispatcher::default();
        let mut parser = Parser::new();
        parser.advance(&mut dispatcher, b"\x1b_Ga=q,i=1;\x1b\\");

        assert_eq!(dispatcher.dispatched[0], Sequence::ApcStart);
        let payload: Vec<u8> = dispatcher
            .dispatched
            .iter()
            ...
            .collect();
        assert_eq!(payload, b"Ga=q,i=1;");
        assert!(dispatcher.dispatched.contains(&Sequence::ApcEnd));
    }

    #[test]
    fn parse_sos_does_not_deliver_apc() {
        let mut dispatcher = Dispatcher::default();
        let mut parser = Parser::new();
        parser.advance(&mut dispatcher, b"\x1bXhello\x1b\\");
        assert!(!dispatcher.dispatched.iter().any(|s| matches!(
            s,
            Sequence::ApcStart | Sequence::ApcPut(_) | Sequence::ApcEnd
        )));
    }
