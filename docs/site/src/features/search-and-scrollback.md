...
Mouse wheel and touchpad scrolling move through scrollback locally, scaled by `scrollback.multiplier` (default `3.0` lines per tick). `scrollback.lines` caps how much history is kept (default `10000`; `0` disables scrollback entirely), or set `scrollback.unlimited = true` to keep everything.
